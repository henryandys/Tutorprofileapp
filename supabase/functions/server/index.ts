...
const EMAIL_TEMPLATES: Record<string, (data: any) => { subject: string; html: string }> = {
  new_booking: (d) => ({
    ...
    html: `<p>Hi ${h(d.tutorName)},</p>
           <p><strong>${h(d.studentName)}</strong> has sent you a lesson request for <strong>${h(d.subject)}</strong>.</p>
           ${d.message ? `<blockquote style="border-left:3px solid #2563eb;padding-left:12px;color:#555">${h(d.message)}</blockquote>` : ""}
           <p><a href="${h(d.appUrl)}/my-profile">View the request</a></p>`,
  }),
  booking_accepted: (d) => ({
    subject: `Your lesson request was accepted!`,
    html: `<p>Hi ${h(d.studentName)},</p>
           <p>Your lesson request for <strong>${h(d.subject)}</strong> with <strong>${h(d.tutorName)}</strong> has been <strong style="color:green">accepted</strong>.</p>
           <p>You can now message your tutor directly. <a href="${h(d.appUrl)}/profile">View your lesson requests</a></p>`,
  }),
  booking_declined: (d) => ({
    subject: `Update on your lesson request`,
    html: `<p>Hi ${h(d.studentName)},</p>
           <p>Your lesson request for <strong>${h(d.subject)}</strong> with <strong>${h(d.tutorName)}</strong> has been declined.</p>
           <p><a href="${h(d.appUrl)}/search">Find another tutor</a></p>`,
  }),
  new_message: (d) => ({
    subject: `New message from ${h(d.senderName)}`,
    html: `<p>Hi there,</p>
           <p>You have a new message from <strong>${h(d.senderName)}</strong> regarding <strong>${h(d.subject)}</strong>.</p>
...
// ── JWT auth middleware ───────────────────────────────────────
// All routes below this point require a valid Supabase user JWT.
// The anon key is NOT accepted — only a signed-in user's access_token.
app.use("/make-server-3c6c6b51/*", async (c, next) => {
